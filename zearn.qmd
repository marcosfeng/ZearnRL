...
```{r}
#| label: tbl-CBM-HBI
#| tbl-cap: "Comparison of model performance using Hierarchical Bayesian Inference (HBI). The table presents mean Bayesian Information Criterion (BIC) scores for different models, all featuring the 'Pedagogical Knowledge' action component, with various reward types. BIC quantifies the models' fit to the data, penalizing for model complexity, with lower values indicating better performance. The baseline model is an intercept-only regression model."
...
for (wrapper in c("2", "6", "10", "14")) {
  ...
  cb_file <- list.files(path = model_folder_path,
                        ...
                        full.names = TRUE)
  qs_file <- list.files(path = model_folder_path,
                        pattern = paste0("hbi_compare_wrapper_",
                                         wrapper, "_model_4.mat"),
                        full.names = TRUE)
  bl_file <- list.files(path = model_folder_path,
                        pattern = "hbi_compare_wrapper_2_model_5.mat",
                        full.names = TRUE)

  q_data <- readMat(q_file)
  lg_data <- readMat(lg_file)
  cb_data <- readMat(cb_file)
  qs_data <- readMat(qs_file)
  ...
  model_comparison_df <- rbind(model_comparison_df, data.frame(
    ...
    Q_simple = mean(qs_data[["cbm"]][[5]][[9]], na.rm = TRUE),
...
model_comparison_df %>%
  ...
  fmt_number(columns = c(
    Q_learning, Lau_Glimcher, Cockburn,
    Q_simple, Baseline
    ), decimals = 3) %>%
  cols_label(
...
<!-- # Analyze performance split -->
<!-- # This function examines how teacher behavior changes before and after identified change points -->
<!-- # It helps in understanding adaptations in teaching strategies -->
