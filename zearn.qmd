...
df_bin <- as.data.table(
  df %>%
    mutate(across(dplyr::starts_with(action), ~if_else(.>median(.),1,0)))
)
# Create lags
...
top_fits_df <- do.call(rbind, lapply(results, function(x) {
  ...
  top_fits <- x$perform_df %>%
    mutate(keep = case_when(!in_hdr(n_train) ~ F,
                            ...
                            .default = T)) %>%
    filter(keep) %>%
    summarize(across(c(auc_out, auc_in, logLik_ind),
...
top_fits_df %>%
  ...
  rename("Total AUC" = auc,
         "Total BIC (x10<sup>3</sup>)" = bic,
         "Best-Fit AUC" = auc_out,
         "Best-Fit Log Likelihood" = logLik_ind) %>%
...
selected_models <- bind_rows(
  top_fits_df %>% filter(State == "None") %>%
    arrange(bic) %>% slice_head(n = n_select),
  top_fits_df %>% filter(State == "None") %>%
    arrange(desc(auc_out)) %>% slice_head(n = n_select),
  top_fits_df %>% filter(State == "None") %>%
    arrange(desc(auc)) %>% slice_head(n = n_select),
  top_fits_df %>% filter(State == "None") %>%
    arrange(desc(logLik_ind)) %>% slice_head(n = n_select),
  top_fits_df %>% filter(State != "None") %>%
    arrange(bic) %>% slice_head(n = 2*n_select),
  top_fits_df %>% filter(State != "None") %>%
    arrange(desc(auc_out)) %>% slice_head(n = 2*n_select),
  top_fits_df %>% filter(State != "None") %>%
    arrange(desc(auc)) %>% slice_head(n = 2*n_select),
  top_fits_df %>% filter(State != "None") %>%
    arrange(desc(logLik_ind)) %>% slice_head(n = 2*n_select)
) %>% distinct(Action, Reward, State, .keep_all = TRUE)

plot <- selected_models %>%
  select(Action, Reward, State, bic, auc_out) %>%
  ggplot(aes(x = auc_out, y = bic, shape = State, color = Reward)) +
  geom_point() +
  ...
  theme_minimal() +
  labs(x = "Best-Fit AUC", y = "Total BIC",
       title = "Selected Models for Further Analysis")
...
filtered_results <- lapply(results, function(x) {
  temp <- data.frame(
    Action = x$Method,
    Reward = x$Reward,
    State = ifelse(is.null(x$State), "None", x$State)
  ) %>%
  mutate(across(c(Action, Reward, State),
                ~ case_when(. == "Frobenius.NNDSVD_student1" ~ "Completion",
                            . == "Frobenius.NNDSVD_student2" ~ "Struggle",
...
model_summary <- do.call(rbind, lapply(filtered_results, function(x) {
  ...
  summary <- summary %>%
    ...
    mutate(across(c(lag1, lag2, rwd1_lag1, rwd1_lag2, rwd2_lag2, st, st_lag1,
                    auc_ind, logLik_ind),
                  ~ lapply(., mean, na.rm = T),
                  .names = "{.col}.mean")) %>%
    mutate(across(c(lag1, lag2, rwd1_lag1, rwd1_lag2, rwd2_lag2, st, st_lag1),
    ...
    mutate(across(c(lag1, lag2, rwd1_lag1, rwd1_lag2, rwd2_lag2, st, st_lag1),
                  ...
                  .names = "{.col}.pos")) %>%
    select(-c(lag1, lag2, rwd1_lag1, rwd1_lag2, rwd2_lag2, st, st_lag1,
...
model_summary_long %>%
  ...
  mutate(statistic = factor(statistic,
         ...
         variable = factor(variable,
                           levels = c(
                             # "lag1", "lag2",
                             "rwd1_lag1", "rwd1_lag2", "rwd2_lag2",
                             "st", "st_lag1",
                             "n_teachers"),
                           labels = c(
                             # "Action<sub>t-1</sub>",
                             # "Action<sub>t-2</sub>",
                             "Reward<sub>t-1</sub> x \n Action<sub>t-1</sub>",
                             "Reward<sub>t-1</sub> x \n Action<sub>t-2</sub>",
                             "Reward<sub>t-2</sub> x \n Action<sub>t-2</sub>",
                             "State<sub>t</sub>",
                             "State<sub>t</sub> x \n Action<sub>t-1</sub>",
                             "No. Teachers"
                             ))) %>%
...
plot_list <- lapply(top_results, function(object) {
  model <- object[[6]]
...
TO-DO: temporal analysis of teachers choices and a bunch of variables: bayesian analysis of <!-- Panel Regression of all variables and badges. --> <!-- Investigate the basic analysis of behavior and its structure over time. John: "Really try to dig into what the structure is between how teachers' choices are changing across weeks, and what factors might be related to those." --> <!-- Detailed Data Analysis by School and Classroom: how much do individual schools and classrooms vary. You might want to downsample schools in some way. --> <!-- Cooper: "What is going on in the data—school quality, what is changing over time, etc. just understand the statistics of all the data." -->
