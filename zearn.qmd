...
df_pca <- as.data.frame(df_comp) %>%
  ...
  mutate(across("RD.elementary_schedule":"RD.grade_level_teacher_materials", ~log1p(.)))
...
def nmf_method(n, method, initial, X_scaled):
    ...
    nmf = NMF(n_components=n, init=initial, beta_loss=method, solver=solv, max_iter=4_000)
...
def reshape_data(df, num_samples, num_timesteps, num_features):
