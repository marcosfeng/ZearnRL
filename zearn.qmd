...
To analyze Zearn data spanning an entire academic year, we first establish a framework for actions and rewards. In this framework, teacher activities drive the educational process, student achievements result from these efforts, and the constantly changing educational environment represents the states. Instead of relying solely on one analytical approach, our strategy involves a large set of candidate models, progressively escalating in complexity, as shown in @tbl-methods. Our overarching goal was to strengthen the reliability of our findings and offer a detailed understanding of the underlying behavioral patterns.
...
More specifically, we select one teacher component as the action and one student component as the reward. When constructing state-based models, we incorporate an additional student component as the state variable. This choice yields 16 models (4 possible actions and 4 possible rewards).
...
load_predictions <- function(file_path) {
  ...
  if(grepl("q_model", file_path)) q_values <- mat_data[["cbm"]][[5]][[12]]
...
plot_reward_seeking <- function(sim_data, wrapper, 
  ...
  smooth_data <- sim_data %>%
    filter(q_val_1_percentile >= lower_bound,
  ...
  p <- ggplot(smooth_data) +
    geom_smooth(aes(x = q_val_1_percentile,
  ...
  for (model in models) {
    ...
    p <- p + geom_smooth(aes(x = q_val_1_percentile,
                             y = !!sym(y_col)), 
                         method = "glm",
    ...
    last_point <- smooth_data %>%
      filter(q_val_1_percentile == max(q_val_1_percentile)) %>%
      summarise(x = max(q_val_1_percentile),
...
all_reward_seeking_plots <- map(c(2, 14), function(wrapper) {
  full_data <- all_predictions[[
    ...
    full_join(all_predictions[[
      paste0("hbi_compare_wrapper_", 2, "_model_", 5, ".mat")
    ]], by = c("Classroom.ID", "week", "action"))
...
plot_uncertainty_aversion <- function(sim_data, wrapper, 
  ...
  sim_data <- sim_data %>%
    ...
    mutate(
      ...
      choice_uncertain_pred_1 = if_else(lag_sd > 0,
                                        pred_2_1,
                                        1 - pred_2_1),
      choice_uncertain_pred_2 = if_else(lag_sd > 0,
                                        pred_2_2,
                                        1 - pred_2_2),
      choice_uncertain_pred_3 = if_else(lag_sd > 0,
                                        pred_2,
                                        1 - pred_2)
    ) %>%
    ungroup()
  ...
  plot_data <- sim_data %>%
    ...
    summarise(
      ...
      choice_uncertain_pred_1 = mean(
        choice_uncertain_pred_1, na.rm = TRUE),
      se_pred_2 = sd(choice_uncertain_pred_2, na.rm = TRUE) / sqrt(n()),
      choice_uncertain_pred_2 = mean(
        choice_uncertain_pred_2, na.rm = TRUE),
      se_pred_3 = sd(choice_uncertain_pred_3, na.rm = TRUE) / sqrt(n()),
      choice_uncertain_pred_3 = mean(
        choice_uncertain_pred_3, na.rm = TRUE)
    ) %>%
    filter(!is.na(ev_bin))
  ...
  for (model in models) {
    if (model == "Baseline") {
      y_col <- "choice_uncertain_pred_3"
    } else if (model == "Lau & Glimcher") {
      y_col <- "choice_uncertain_pred_2"
    } else if (model == "Q-learning") {
      y_col <- "choice_uncertain_pred_1"
    }
...
plot_prediction_errors <- function(sim_data, wrapper) {
  ...
  parameters <- load_parameters(
    paste0("CBM/zearn_results/hbi_results/hbi_compare_wrapper_",
...
plot_action_percentages <- function(sim_data, wrapper, y_lim = c(0.13, 0.5)) {
  ...
  plot_data <- sim_data %>%
    ...
    mutate(
      ...
      q_action = pred_2_1,  # Q-learning prediction
      lau_glimcher_action = pred_2_2,  # Lau & Glimcher prediction
      baseline_action = pred_2  # Baseline model prediction
    ) %>%
    ungroup() %>%
    group_by(biweek) %>%
    summarise(
