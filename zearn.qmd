...
create_formula <- function(method, lag, comp) {
  return(as.formula(
    paste0(method, comp, " ~ ",
           ...
           paste0("Badges.per.Active.User", "_", 1:lag, collapse = " + "),
           " + ", "week_lag")))
}
...
methods <- c("PCA", "FrobeniusNNDSVD", "FrobeniusNNDSVDA", "Kullback.Leibler", "Autoencoder")
...
methods_for_plots <- c("FrobeniusNNDSVD", "Kullback.Leibler", "FrobeniusNNDSVDA")
...
nll_data <- filter(plot_data, !is.na(avg_nloglik)) %>% 
  mutate(value = avg_bic, metric = "Negative Log-Likelihood")
...
fit_model <- function(formula, data) {
  model <- brm(formula, data = data, family = bernoulli(), cores = 3)
  return(model)
}

df_subset <- read.csv(file = "./Bayesian/df_subset.csv")
setDT(df_subset)
df_subset$state <- df_subset$state - 1
for (col in paste0("FrobeniusNNDSVD", 1:3, "bin")) {
  df_subset <- get_lag_value(df_subset, col, 1)
}
...
models_nh <- lapply(formulas_nh, fit_model, data = df_subset)
...
models_h <- lapply(formulas_h, fit_model, data = df_subset)
