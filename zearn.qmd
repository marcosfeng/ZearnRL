...
The Q-function, $Q(s, a)$, is defined for all state-action pairs $(s, a)$, where $s$ is the state and $a$ is the action. The Q-function represents the expected return or future reward for taking action $a$ in state $s$ following a certain policy $\pi = P(a)$. The Q-function is updated iteratively using the Bellman equation as follows:
...
+--------------------------+-----------------------------------------------------------------------------+---------------------------------------------------------------+
| Analytical Methods       | Q-learning, Actor-Critic Model Estimation                                   | R, Matlab (`CBM` package for Laplace approximation)           |
+--------------------------+-----------------------------------------------------------------------------+---------------------------------------------------------------+
...
: Analytical steps employed in the study. For detailed information on software and tools, please refer to @sec-software-tools. {#tbl-methods}
...
We use the Hierarchical Bayesian Inference (HBI) framework, as described by Piray et al. (2019), to assess the fitness of our RL models and estimate their respective parameters across subjects. This approach leverages population-level distributions to inform individual parameter variation. This method leverages Laplace approximations for efficient computation of model evidence and parameter estimation by approximating the integrals involved in Bayesian inference. For a given model $k$ and subject $n$, we assume that individual parameters ($h_{k,n}$) follow a normal distributionwithin the population, i.e., $p(h_{k,n}) = N(h_{k,n} | \mu_k, V_k)$, where $\mu_k$ and $V_k$ represent the mean and variance of the prior distribution over $h_{k,n}$, respectively.

We then use an expectation-maximization algorithm, iteratively performing two critical steps:

1.  Expectation Step: Individual parameters ($h_{k,n}$) are estimated considering the group-level distribution.
2.  Maximization Step: Group parameters ($\mu_k$ and $V_k$) are updated based on current individual parameter estimates. The update for $\mu_k$ is exemplified by the average of subject-level mean estimates ($\theta_{k,n}$) across all subjects, adhering to the formula: $\mu_k = \frac{1}{N}\sum_{n}\theta_{k,n}$, where $N$ is the total number of subjects.

With this approach, we can estimate the log-likelihood for each subject's data, given the proposed models, and the following model parameters:

1.  Q-learning:
    -   Learning Rate: $\sigma(\alpha)$
    -   Discount Rate: $\sigma(\gamma)$
    -   Temperature: $\exp(\tau)$
    -   Cost: $\exp(\text{cost})$
2.  Actor-Critic:
    -   Weights $w$ and $\theta$: $\exp(w)$, $\exp(\theta)$
    -   Learning rates ($\alpha_w$, $\alpha_\theta$): $\sigma(\alpha_w)$, $\sigma(\alpha_\theta)$
    -   Discount Factor: $\sigma(\gamma)$
    -   Temperature: $\exp(\tau)$
...
-   Classification of Individual Responses: Each subject is classified according to their best-fit model, accounting for individual variation from the hierarchical Bayesian framework.

-   Parameter Estimation Across Models: We also use the parameter estimates for each subject.

-   Exploration of Group-Level Trends: We aggregate teacher data by their respective schools and test for overarching trends across groups.

-   Variables of interest: We test the effect of school income levels, classroom size, etc., on teacher classification and parameter estimates.
...
## Software and Tools {#sec-software-tools}
