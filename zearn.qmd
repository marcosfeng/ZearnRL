...
| Feature Selection, Analytical Methods | Q-learning, Actor-Critic Model Estimation | R, Matlab (`CBM` package for Laplace approximation) [@piray2019] |
...
All statistical analyses were performed using the `lmtest` package (Zeileis & Hothorn, 2002) for clustered standard errors and the sandwich package (Zeileis, 2004) for robust covariance matrix estimation.
...
#| fig-cap: "Distribution of teacher-specific Akaike Information Criteria (AIC). The histogram displays the spread of individual teachers' log-likelihoods, with higher values indicating better fit. The dashed line represents the median log-likelihood across all teachers."
