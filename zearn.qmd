...
In this section, we delve into the process of estimating the reinforcement learning (RL) models based on the selected criteria of model performance. By leveraging the MATLAB computational environment, we have executed a rigorous model fitting procedure that is tailored to understand the teaching strategies embedded within our data set.

We first preset the random number generator for reproducibility and load the essential data for analysis. To account for the multifaceted nature of our models, we use a parallel computing framework to estimate each model, ensuring a comprehensive search over the parameter space.

After estimating a suite of models, we retrieve the log evidence—a measure of model fit that takes into account the complexity of each model. This evidence is used to rank the models according to their performance.

#### Distribution of Model Performance

The distribution of log evidence scores provides insight into the overall performance of our model suite. We present this distribution to showcase the variance in fit across all estimated models.

```{r}
#| label: fig-log-evidence-histogram
#| fig-cap: "Histogram of Log Evidences. "
...
Next, we present the four models that distinguished themselves during the initial estimation phase. These models were selected based on their top log evidence scores, indicating a strong balance between goodness-of-fit and model simplicity.

```{r}
#| label: fig-top-CBM
#| fig-cap: "Top Model Log Evidences"
...
To further refine our selection, we employ Hierarchical Bayesian Inference (HBI) to perform a more sophisticated model comparison. This statistical approach allows us to compare models not just by their individual fits, but also by their ability to explain data across different subjects.

The HBI results and corresponding model comparison statistics serve as our basis for selecting the most appropriate RL model. By integrating across all levels of the data hierarchy, we gain a deeper understanding of the models' explanatory power.
...
for(param in unique(parameters_long$Parameter)) {
  ...
  p <- ggplot(data_current, aes(x = Parameter, y = Value)) +
    geom_violin(
