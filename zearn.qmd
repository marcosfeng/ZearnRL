...
compute_nloglik <- function(residuals, n) {
...
# Adding Minutes and Logins to results_list
results_list$Minutes <- list(df$tch_min)
results_list$Logins <- list(df$User.Session)

# Preparing the results data frame
results_df <- data.frame(Method = character(),
                         N_components = numeric(),
                         bic_Minimum = numeric(),
                         bic_Mean = numeric(),
                         nloglik_Minimum = numeric(),
                         nloglik_Mean = numeric(),
...
for (method in methods) {
  ...
  results <- foreach(comp = 1:n_comp,
    ...
    # Models
    wi <- plm(formula, data = train_data, effect = "twoway", model = "within")
    re <- plm(formula, data = train_data, effect = "twoway", model = "random")
    model <- model_selection(wi, re)
    ...
    list(nloglik = compute_nloglik(residuals, n),
  ...
  results_df <- rbind(results_df, data.frame(Method = method,
                                             N_components = n_comp,
                                             bic_Minimum = min(sapply(results, `[[`, "bic")),
                                             bic_Mean = mean(sapply(results, `[[`, "bic")),
                                             nloglik_Minimum = min(sapply(results, `[[`, "nloglik")),
                                             nloglik_Mean = mean(sapply(results, `[[`, "nloglik")),
                                             stringsAsFactors = FALSE))
}

# Wrangling the results data frame for plotting
df_bic <- results_df %>%
  pivot_longer(cols = c(bic_Minimum, bic_Mean), names_to = "type", values_to = "BIC") %>%
  mutate(type = str_replace(type, "bic_", ""))
# Convert type column to a factor for ordering
df_bic$type <- factor(df_bic$type, levels = c("Minimum", "Mean"))

df_nll <- results_df %>%
  pivot_longer(cols = c(nloglik_Minimum, nloglik_Mean), names_to = "type", values_to = "NLL") %>%
  mutate(type = str_replace(type, "nloglik_", ""))
...
# Plot BIC values
p1 <- ggplot(df_bic, aes(x = Method, y = BIC, fill = Method)) +
  geom_col(data = subset(df_bic, type == "Mean"), width = 0.6, position = position_dodge()) +
  geom_point(data = subset(df_bic, type == "Minimum"),
             position = position_dodge(0.6), size = 3, color = "black") +
  labs(
    title = "In-Sample BIC",
    x = "Method",
    y = "BIC",
...
# Plot NLL values
p2 <- ggplot(df_nll, aes(x = Method, y = NLL, fill = Method)) +
  geom_col(data = subset(df_nll, type == "Mean"), width = 0.6, position = position_dodge()) +
  geom_point(data = subset(df_nll, type == "Minimum"),
             position = position_dodge(0.6), size = 3, color = "black") +
  labs(
...
n_lags = 8
n_lags_short = 4
columns <- c("tch_min", "KullbackLeibler1", "KullbackLeibler2", "KullbackLeibler3",
             "Badges.per.Active.User")
...
### Component 1
formula <- as.formula("KullbackLeibler1 ~ Badges.per.Active.User_1 + KullbackLeibler1_1")
# First test which model is appropriate
# Random or Fixed Effects by the Hausman test:
hausman <- phtest(KullbackLeibler1 ~ Badges.per.Active.User_1 + KullbackLeibler1_1,
                  data = df_panel)
if (hausman$p.value < 0.05) {
  panel_KL1 <- plm(formula,
                     data = df_panel,
                     model = "within")
} else {
  panel_KL1 <- plm(formula, 
...
### Component 2
formula <- as.formula("KullbackLeibler2 ~ Badges.per.Active.User_1 + KullbackLeibler2_1")
hausman <- phtest(KullbackLeibler2 ~ Badges.per.Active.User_1 + KullbackLeibler2_1,
                  data = df_panel)
if (hausman$p.value < 0.05) {
  panel_KL2 <- plm(formula,
                     data = df_panel,
                     model = "within")
} else {
  panel_KL2 <- plm(formula, 
...
### Component 3
formula <- as.formula("KullbackLeibler3 ~ Badges.per.Active.User_1 + KullbackLeibler3_1")
hausman <- phtest(KullbackLeibler3 ~ Badges.per.Active.User_1 + KullbackLeibler3_1,
                  data = df_panel)
if (hausman$p.value < 0.05) {
  panel_KL3 <- plm(formula,
                    data = df_panel,
                    model = "within")
} else {
  panel_KL3 <- plm(formula, 
...
df_coeffs_long <- full_join(df_estimates,
  ...
  mutate(Action = factor(variable, 
                         levels = c("KullbackLeibler1", "KullbackLeibler2", "KullbackLeibler3",
                                    "tch_min", "User.Session",
                                    "Badges.per.Active.User"),
                         labels = c("Kullback-Leibler1", "Kullback-Leibler2", "Kullback-Leibler3",
...
pca1_model_bic <- BIC(panel_KL1)
pca2_model_bic <- BIC(panel_KL2)
pca3_model_bic <- BIC(panel_KL3)
