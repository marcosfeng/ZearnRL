...
def nmf_method(n, method, initial, X_scaled, data_label, solv = 'mu', nomin = False):
  ...
  residuals.setdefault(f"{method_name}_{data_label}", {})[n] = np.sum((X_scaled.values - X_hat)**2)
  ...
  if n == 4:
    ...
    for i in range(n):
        contribution = np.sum(X_nmf[:, i][:, np.newaxis] * nmf_comp[i, :])
        reconstruction_contribution.append(contribution)
    reconstruction_contribution = np.array(reconstruction_contribution) / np.sum(reconstruction_contribution)
      
    # Store the results
    results.setdefault(f"{method_name}_{data_label}_reconstruction_contribution", {})[n] = reconstruction_contribution.tolist()
...
def perform_pca_nmf(X_scaled, data_label, n_comp):
  for n in range(2, n_comp):
    ...
    residuals.setdefault(f"PCA_{data_label}", {})[n] = np.sum((X_scaled.values - X_hat)**2)
...
reconstruction_teachers <- py$results[["Frobenius NNDSVD_nomin_teachers_reconstruction_contribution"]][["4"]]
reconstruction_students <- py$results[["Frobenius NNDSVD_nomin_students_reconstruction_contribution"]][["4"]]

# Arrange the rows by the magnitude of each component
names(df_heatmap_teachers) <- paste0("Comp ", 1:4)
names(df_heatmap_students) <- paste0("Comp ", 1:4)
...
# Add the reconstruction contribution to the column names
names(df_heatmap_teachers) <- paste0("Comp ", 1:4, "\n(", round(reconstruction_teachers*100, 1), "%)")
names(df_heatmap_students) <- paste0("Comp ", 1:4, "\n(", round(reconstruction_students*100, 1), "%)")
...
load_parameters <- function(file_path, transformed = TRUE) {
...
<!-- fig-parameter-correlations -->

```{r}
#| label: fig-parameter-correlations
#| fig-cap: "Correlation between individual parameter estimates from non-hierarchical (Matlab) and hierarchical (Stan) estimation approaches. Each point represents one teacher. Correlation values are Spearman's rho, with asterisks indicating p < 0.001."

# Load Stan results 
stan_fit <- readRDS("Bayesian/Results/ql_opt.RDS")

# Extract individual parameters from Stan fit
stan_params <- stan_fit$draws(format = "df") %>%
  select(starts_with(c("alpha[", "gamma[", "tau[", "cost[", "ev_init["))) %>%
  ...
  pivot_longer(everything(), 
              ...
              values_to = "stan_value")

# Load Matlab parameters using existing function
matlab_params <- load_parameters(
  "CBM/zearn_results/aggr_results/lap_aggr_q_model_2.mat"
) %>%
  pivot_longer(-Classroom.ID,
               names_to = "parameter",
               values_to = "matlab_value")

# Join the estimates
param_comparison <- stan_params %>%
  mutate(id = as.integer(id)) %>%
  left_join(matlab_params %>%
              group_by(parameter) %>%
              mutate(id = row_number()) %>%
              ungroup(),
            by = c("id", "parameter"))

# Calculate correlations and create labels
param_cors <- param_comparison %>%
  group_by(parameter) %>%
  summarise(
    correlation = cor(matlab_value, stan_value, method = "spearman"),
    p_value = cor.test(matlab_value, stan_value, method = "spearman")$p.value,
...
ggplot(param_comparison, 
       aes(x = matlab_value, y = stan_value)) +
  geom_point(size = 0.5, alpha = 0.3) +
  geom_text(data = param_cors,
            aes(label = label),
            x = -Inf, y = Inf,
            hjust = -0.2, vjust = 1) +
  facet_wrap(~parameter, scales = "free",
             labeller = as_labeller(c(
              ...
              tau = "Inv. Temperature",
              cost = "Cost",
              ev_init = "Initial Q-value"
            ))) +
  labs(x = "Non-hierarchical Estimate",
...
```{r prep-stan-behavioral}

# Function to calculate Q-values and prediction errors using Stan parameters  
calculate_stan_learning <- function(data) {
  data %>%
    group_by(Classroom.ID) %>%
    arrange(week) %>%
    mutate(
      ...
      # Compute week differences like Stan
      week_diff = c(0, diff(week)),
      
      # Set initial Q-value
      q_val = ev_init
    ) %>%
    group_modify(function(classroom_data, classroom_id) {
      n_trials <- nrow(classroom_data)
      
      # Loop through trials to calculate Q-values and PEs sequentially
      for(t in 2:n_trials) {
        gamma_power <- classroom_data$gamma[1]^classroom_data$week_diff[t]
        reward_discounted <- gamma_power * classroom_data$reward[t]
        
        if(classroom_data$action[t-1] == 1) {
          delta <- reward_discounted - classroom_data$q_val[t-1]
          classroom_data$pe[t] <- delta
          classroom_data$q_val[t] <- classroom_data$q_val[t-1] + 
            classroom_data$alpha[1] * delta
        } else {
          classroom_data$pe[t] <- -reward_discounted
          classroom_data$q_val[t] <- classroom_data$q_val[t-1] - 
            classroom_data$alpha[1] * reward_discounted
        }
      }
      classroom_data
    }) %>%
    ungroup() %>%
    mutate(
      pred = 1/(1+exp(-tau * (q_val - cost)))
    )
}

# Get parameters in wide format for predictions
stan_params_wide <- param_comparison %>%
  select(-matlab_value) %>%
  pivot_wider(names_from = parameter, 
              values_from = stan_value,
              id_cols = id)

# Get predictions from Stan fit
stan_predictions <- stan_fit$summary(variables = "y_pred", "mean") %>%
  pull(mean)

# Calculate Q-values and prediction errors for Stan estimates
sim_data_stan <- all_predictions[[
  paste0("lap_aggr_q_model_2.mat")
  ]] %>%
  mutate(classroom_idx = as.numeric(factor(Classroom.ID))) %>%
  select(-c("pred_0","pred_1","q_val")) %>%
  left_join(stan_params_wide, by = c("classroom_idx" = "id")) %>%
  group_split(Classroom.ID) %>%
  map_dfr(~calculate_stan_learning(.x)) %>%
  mutate(
    stan_pred = stan_predictions,
    pred_diff = abs(pred - stan_pred)
  )
...
``` {r}
#| label: fig-reward-seeking-stan
#| fig-cap: "Reward-seeking behavior based on hierarchical model estimates. The x-axis represents the percentile of the difference in Q-values between action and inaction. The y-axis shows the proportion of times teachers chose to act."

plot_reward_seeking_stan <- function(sim_data, 
                                   lower_bound = 0.05,
                                   upper_bound = 0.95,
                                   y_limits = c(0.25,0.45)) {
  
  # Calculate EV and Uncertainty using Stan estimates
  sim_data <- sim_data %>%
    group_by(Classroom.ID) %>%
    mutate(
      q_val_percentile = percent_rank(q_val)
    ) %>%
    ungroup()
  ...
  plot_data_qv <- sim_data %>%
    ...
    summarise(
      mean_choice = mean(action, na.rm = TRUE),
      se_choice = sd(action, na.rm = TRUE) / sqrt(n()),
      mean_q_diff = mean(q_val, na.rm = TRUE),
      mean_q_diff_perc = mean(q_val_percentile, na.rm = TRUE),
      mean_pred = mean(stan_pred, na.rm = TRUE),
      se_pred = sd(stan_pred, na.rm = TRUE) / sqrt(n()),
      mean_pred_bl = mean(baseline_pred, na.rm = TRUE),  # Add baseline predictions
      se_pred_bl = sd(baseline_pred, na.rm = TRUE) / sqrt(n())
    ) %>%
    filter(!is.na(q_bin))
  ...
  model_colors <- c(
    "Data" = "black", 
    "Q-learning" = brewer.pal(8, "Set2")[2],
    "Baseline" = brewer.pal(8, "Set2")[8])  # Add baseline color
  ...
  # Add Q-learning model predictions
  p <- p + geom_smooth(aes(x = q_val_percentile,
                          y = stan_pred), 
                      method = "glm",
                      method.args = list(family = "quasibinomial"),
                      se = TRUE, alpha = 0.2,
                      color = model_colors["Q-learning"],
                      fill = model_colors["Q-learning"])

  # Add baseline model predictions  
  p <- p + geom_smooth(aes(x = q_val_percentile,
                          y = baseline_pred),
                      method = "glm", 
                      method.args = list(family = "quasibinomial"),
  ...
  # Get last points for labels
  last_points <- bind_rows(
    smooth_data %>%
      filter(q_val_percentile == max(q_val_percentile)) %>%
      summarise(x = max(q_val_percentile),
                y = mean(stan_pred, na.rm = TRUE),
                model = "Q-learning"),
    smooth_data %>%
      filter(q_val_percentile == max(q_val_percentile)) %>%
      summarise(x = max(q_val_percentile),
                y = mean(baseline_pred, na.rm = TRUE),
                model = "Baseline")
  )
...
# Get baseline predictions and add them to sim_data_stan
bl_fit <- readRDS("Bayesian/Results/bl_opt.RDS")
baseline_predictions <- bl_fit$summary(variables = "y_pred", "mean") %>%
  pull(mean)

sim_data_stan <- sim_data_stan %>%
  mutate(baseline_pred = baseline_predictions)

# Create plot using Stan estimates
plot_reward_seeking_stan(sim_data_stan)
```

```{r reward-seeking-corr-stan}
...
get_slope_ci <- function(data, y_var) {
  ...
  model <- glm(as.formula(paste(y_var, "~ q_val_percentile")),
               family = model_fam,
               data = data)
  ...
  # Extract coefficients and calculate CI
  coef_summary <- coeftest(model, vcov. = vcov_cluster)
  estimate <- coef_summary["q_val_percentile", "Estimate"]
  se <- coef_summary["q_val_percentile", "Std. Error"]
  ci_lower <- estimate - 1.96 * se
  ci_upper <- estimate + 1.96 * se
...
# Calculate percentiles and prepare data
sim_data_corr <- sim_data_stan %>%
  group_by(Classroom.ID) %>%
  mutate(q_val_percentile = percent_rank(q_val)) %>%
  ungroup()

# Calculate slopes for all models
slopes <- tibble(
  model = c("Actual", "Q-learning", "Baseline"),
  slope_data = list(
    get_slope_ci(sim_data_corr, "action"),
    get_slope_ci(sim_data_corr, "stan_pred"),
    get_slope_ci(sim_data_corr, "baseline_pred")
  )
) %>%
  unnest(slope_data)
...
```{r}
#| label: fig-uncertainty-aversion-stan
#| fig-cap: "Uncertainty aversion comparing hierarchical models. The graphs compare Q-learning and baseline hierarchical models in their ability to capture teachers' uncertainty-related behavior. The x-axis represents the percentile of the difference in expected value (EV) between uncertain and certain options. The y-axis shows the proportion of times teachers chose the uncertain option. The horizontal dashed line at 50% represents indifference between certain and uncertain options."

# Function to calculate EV and uncertainty
calculate_ev_uncertainty <- function(df) {
  # Calculate values for action = 0
  v_0 <- df$reward
  v_0[df$action > 0] <- NA
  if (is.na(v_0[1])) v_0[1] <- 0

  # Calculate values for action = 1
  v_1 <- df$reward
  v_1[df$action == 0] <- NA
  if (is.na(v_1[1])) v_1[1] <- 0
...
# Function to plot uncertainty aversion
plot_uncertainty_aversion <- function(sim_data, 
                                    lower_bound = 0.05,
                                    upper_bound = 0.95,
                                    y_limits = c(0.35, 0.65)) {
  ...
  sim_data <- sim_data %>%
    group_by(Classroom.ID) %>%
    group_modify(~{
      ev_unc <- calculate_ev_uncertainty(.x)
      mutate(.x,
             ev = ev_unc$ev,
             sd = ev_unc$uncertainty)
    }) %>%
    mutate(
      lag_ev = c(0, ev[-n()]),
      lag_sd = c(0, replace_na(sd[-n()], 0)),
      ...
      choice_uncertain = as.numeric(xor(action == 0, lag_sd > 0)),
      # choice_uncertain for each model
      choice_uncertain_ql = if_else(lag_sd > 0, stan_pred, 1 - stan_pred),
      choice_uncertain_bl = if_else(lag_sd > 0, baseline_pred, 1 - baseline_pred)
    ) %>%
    summarize(
      delta_ev = mean(delta_ev, na.rm = TRUE),
      choice_uncertain = mean(choice_uncertain, na.rm = TRUE),
      choice_uncertain_ql = mean(choice_uncertain_ql, na.rm = TRUE),
      choice_uncertain_bl = mean(choice_uncertain_bl, na.rm = TRUE)
    ) %>%
    ungroup() %>%
    mutate(
      delta_ev_percentile = percent_rank(delta_ev)
  ...
  plot_data <- sim_data %>%
    ...
    summarise(
      ...
      mean_delta_ev_perc = mean(delta_ev_percentile, na.rm = TRUE),
      choice_uncertain_ql = mean(choice_uncertain_ql, na.rm = TRUE),
      se_ql = sd(choice_uncertain_ql, na.rm = TRUE) / sqrt(n()),
      choice_uncertain_bl = mean(choice_uncertain_bl, na.rm = TRUE),
      se_bl = sd(choice_uncertain_bl, na.rm = TRUE) / sqrt(n())
    ) %>%
    filter(!is.na(ev_bin))
  ...
  # Add model predictions - Q-learning
  p <- p + geom_smooth(aes(x = delta_ev_percentile,
                          y = choice_uncertain_ql), 
                      method = "glm",
                      method.args = list(family = "quasibinomial"),
                      se = TRUE, alpha = 0.2,
                      color = model_colors["Q-learning"],
  ...
  # Add model predictions - Baseline
  p <- p + geom_smooth(aes(x = delta_ev_percentile,
                          y = choice_uncertain_bl), 
                      method = "glm",
                      method.args = list(family = "quasibinomial"),
                      se = TRUE, alpha = 0.2,
                      color = model_colors["Baseline"],
  ...
  last_points <- bind_rows(
    smooth_data %>%
      filter(delta_ev_percentile == max(delta_ev_percentile)) %>%
      summarise(x = max(delta_ev_percentile),
                y = mean(choice_uncertain_ql, na.rm = TRUE),
                model = "Q-learning"),
    smooth_data %>%
      filter(delta_ev_percentile == max(delta_ev_percentile)) %>%
      summarise(x = max(delta_ev_percentile),
                y = mean(choice_uncertain_bl, na.rm = TRUE),
                model = "Baseline")
  )
...
plot_uncertainty_aversion(sim_data_stan)
```

```{r uncertainty-corr-stan}
...
# Prepare data with EVs and uncertainties
full_data <- sim_data_stan %>%
  group_by(Classroom.ID) %>%
  group_modify(~{
    ev_unc <- calculate_ev_uncertainty(.x)
    mutate(.x,
...
# Calculate indifference points for all models
indifference_points <- tibble(
  model = c("Actual", "Q-learning", "Baseline"),
  indifference_data = list(
    get_indifference_point_ci(full_data, "choice_uncertain", "delta_ev"),
    get_indifference_point_ci(full_data, "choice_uncertain_ql", "delta_ev"),
    get_indifference_point_ci(full_data, "choice_uncertain_bl", "delta_ev")
  )
) %>%
  unnest(indifference_data)
...
plot_prediction_errors <- function(sim_data, wrapper) {
  ...
  plot_data <- sim_data %>%
    ...
    mutate(
      ...
      q_pe = case_when(
        lag(action) == 1 ~  gamma^time_diff * reward - lag(q_val),
...
```{r}
#| label: fig-prediction-errors-stan
#| fig-cap: "Learning curves for hierarchical models. The plots show the mean reward prediction errors across teachers over time. The x-axis represents biweekly periods throughout the school year. The solid lines represent mean values and the shaded areas represent the standard error of the mean. A dashed horizontal line at y=0 serves as a reference for zero prediction error."

plot_prediction_errors <- function(sim_data) {
  # Add adjusted week and date for plotting
  plot_data <- sim_data %>%
    group_by(Classroom.ID) %>%
    arrange(Classroom.ID, week) %>%
    mutate(
      # Baseline PE is just deviation from mean prediction
      biweek = ceiling(adjusted_week / 2)
    ) %>%
    ungroup() %>%
    group_by(biweek) %>%
    summarise(
      mean_q_pe = mean(pe, na.rm = TRUE),
      se_q_pe = sd(pe, na.rm = TRUE) / sqrt(n()),
      date = mean(date)
    )
  ...
  # Define color scheme
  colors <- c(
    "Prediction Error" = brewer.pal(8, "Set2")[1]
  )
...
# Generate plot using Stan estimates
plot_prediction_errors(
  sim_data_stan %>%
    group_by(Classroom.ID) %>%
    filter(week == 10) %>%
    pull(Classroom.ID) %>%
    {filter(sim_data_stan, Classroom.ID %in% .)} %>%
    mutate(adjusted_week = week - 8) %>%
    filter(adjusted_week >= 1) %>%
    mutate(date = as_date("2019-08-26") + weeks(adjusted_week - 1))
  )
...
plot_q_value_reward_diff <- function(sim_data, wrapper, window_size = 4) {
  ...
  plot_data <- plot_data %>%
    ...
    mutate(
      ...
      reward_diff = mean(rolling_reward_action - rolling_reward_inaction,
                         na.rm = TRUE),
      se_reward_diff = sd(rolling_reward_action - rolling_reward_inaction,
                          na.rm = TRUE)/sqrt(n())
...
```{r}
#| label: fig-q-value-reward-diff-stan
#| fig-cap: "Q-value and reward differences over time for hierarchical model. The x-axis represents biweekly periods throughout the school year. The y-axis shows the difference in Q-values or mean rewards between action and inaction. Shaded areas represent the standard error of the mean."

plot_q_value_reward_diff <- function(sim_data, window_size = 4) {
  plot_data <- sim_data %>%
    group_by(Classroom.ID) %>%
    arrange(Classroom.ID, week) %>%
    mutate(
  ...
  last_points <- plot_data %>%
    ...
    mutate(
      label = case_when(
        metric == "y_q" ~ "Q-value difference",
        metric == "y_r" ~ "Difference in mean reward"
      )
    )
...
plot_q_value_reward_diff(
  sim_data_stan %>%
    ...
    mutate(adjusted_week = week - 9 + 1) %>%
    filter(adjusted_week >= 1) %>%
    mutate(date = as_date("2019-08-26") + weeks(week - 1))
)
```
...
full_data <- load_parameters(model_path, transformed = FALSE) %>%
  inner_join(
    df_cleaned %>%
...
```{r, results='asis'}
#| label: tbl-optimality-stan
#| tbl-cap: "Impact of Q-learning Model Parameters on Average Weekly Badges Earned per Student. Three linear regression models examine the correlations between a teacher's reinforcement learning (RL) parameters and student engagement, measured by average weekly badges earned per student. Model 1 includes only RL parameters. Model 2 adds controls for LOOIC, number of weeks, total students, and number of classes. Model 3 further incorporates controls for grade level, poverty level, charter school status, and whether the school has a paid Zearn account."

predictors <- c("alpha", "gamma", "tau", "cost", "ev_init")

# Load LOOIC
ql_opt_loo <- readRDS("~/GitHub/ZearnRL/Bayesian/Results/model_comparison.RDS")

stan_params <- stan_fit$draws(format = "df") %>%
  select(starts_with(c("alpha_raw[", "gamma_raw[", "tau_raw[", "cost_raw["))) %>%
  # Calculate mean for each parameter
  summarise(across(everything(), mean)) %>%
  # Reshape to long format
  pivot_longer(everything(), 
...
sim_data_stan <- sim_data_stan %>%
  inner_join(stan_params, by = c("classroom_idx" = "id"))

# Create dataframe for regression
full_data <- sim_data_stan %>%
  group_by(Classroom.ID) %>%
  summarise(
    alpha = first(alpha_raw),
    gamma = first(gamma_raw),
    tau = first(tau_raw),
    cost = first(cost_raw),
    ev_init = first(ev_init),
    .groups = "drop"
  ) %>%
  # Add demographics
  inner_join(
    ...
    by = c("Classroom.ID")) %>%
  # Add LOOIC
  inner_join(sim_data_stan %>%
           pull(Classroom.ID) %>%
           cbind(Classroom.ID = .,
                 looic = ql_opt_loo[["qlearning"]][["mixis"]]) %>%
           as_data_frame() %>%
           group_by(Classroom.ID) %>%
           summarize(LOOIC = mean(looic)),
           by = c("Classroom.ID")) %>%
  mutate(across(all_of(predictors), scale),
         Income = factor(Income, ordered = TRUE), 
         grade = factor(grade, ordered = TRUE,
...
model1 <- lm(`Avg. Badges` ~ alpha + gamma + tau + cost + ev_init,
             data = full_data)
model2 <- lm(`Avg. Badges` ~ alpha + gamma + tau + cost + ev_init +
               LOOIC + `No. of Weeks` + `Total Students` + `No. of Classes`,
             data = full_data)
model3 <- lm(`Avg. Badges` ~ alpha + gamma + tau + cost + ev_init +
               LOOIC + `No. of Weeks` + `Total Students` + `No. of Classes` +
               grade +  Poverty + `Charter School` + `Paid Account`,
...
stargazer(model1, model2, model3,
          ...
          covariate.labels = c("$\\alpha$", "$\\gamma$",
                              ...
                              "No. of Classes",
                              "Charter School", "Paid Zearn Account"),
          omit = c("LOOIC", "grade", "Poverty"),
          dep.var.labels = "Badges",
          star.cutoffs = c(.05, .01, .001), 
          star.char = c("*", "**", "***"),
          add.lines = list(c("Control for LOOIC",
                            "", "Yes", "Yes"),
                          c("Control for Grade Level",
                            "", "", "Yes"),
                          c("Control for Poverty Level",
...
# Calculate number of weeks per classroom
classroom_weeks <- df %>%
  group_by(Classroom.ID) %>%
  summarize(Weeks = n())

# Create the histogram
ggplot(classroom_weeks, aes(x = Weeks)) +
  geom_histogram(
    color = "black",
    fill = "steelblue",
    binwidth = 2,
  ...
  scale_x_continuous(
    breaks = c(1, seq(5, max(classroom_weeks$Weeks), by = 5))
  ) +
  labs(
    x = "Total Number of Effective Weeks",
    y = "Frequency (Classrooms)"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
...
plot_action_percentages <- function(sim_data, wrapper, y_lim = c(0.13, 0.5)) {
  ...
  plot_data <- sim_data %>%
    inner_join(parameters, by = "Classroom.ID") %>%
    mutate(biweek = ceiling(adjusted_week / 2)) %>%
    group_by(Classroom.ID, biweek) %>%
    summarize(
      actual_action = mean(action, na.rm = TRUE),
      q_action = mean(pred_1_ql, na.rm = TRUE),  # Q-learning prediction
      lau_glimcher_action = mean(pred_1_lg, na.rm = TRUE),  # Lau & Glimcher prediction
      baseline_action = mean(pred_1, na.rm = TRUE),  # Baseline model prediction
      date = first(date)
    ) %>%
    ungroup() %>%
    group_by(biweek) %>%
...
for (wrapper in c(2, 14)) {
  full_data <- all_predictions[[
    ...
    full_join(all_predictions[[
      paste0("lap_aggr_baseline_model_", wrapper, ".mat")
    ]] %>% dplyr::select(-reward), by = c("Classroom.ID", "week", "action"))
  full_data <- full_data %>%
    group_by(Classroom.ID) %>%
    filter(week == 10) %>%
    pull(Classroom.ID) %>%
...
```{r}
#| label: fig-action-percentages-balanced
#| fig-cap: "Observed teacher behavior and model predictions over the academic year. The graph shows the percentage of times teachers chose to engage in Pedagogical Content activities. The x-axis represents biweekly periods throughout the school year. The black dashed line represents observed teacher behavior, while colored lines represent predictions from different models. Shaded areas represent the 95% confidence intervals for each model's predictions."

# Create a list to store all plots
action_percentage_balanced_plots <- list()
...
for (wrapper in c(2, 14)) {
  ...
  # Create biweekly periods and standardize dates
  analysis_biweeks <- 1:16
  balanced_data <- full_data %>%
    mutate(
      adjusted_week = week - 8,
      biweek = ceiling(adjusted_week / 2),
      date = as_date("2019-08-26") + weeks(adjusted_week - 1)
    ) %>%
    filter(adjusted_week >= 1)
  
  # Find teachers with at least one observation in each biweekly period
  teacher_completeness <- balanced_data %>%
    filter(biweek %in% analysis_biweeks) %>%
    group_by(Classroom.ID, biweek) %>%
    summarize(has_data = n() > 0, .groups = "drop") %>%
    group_by(Classroom.ID) %>%
    summarize(
      unique_biweeks = sum(has_data),
      expected_biweeks = length(analysis_biweeks),
      is_complete = unique_biweeks == expected_biweeks
    )
  
  # Get list of teachers with complete data
  complete_teachers <- teacher_completeness %>% 
    filter(is_complete == TRUE) %>%
    pull(Classroom.ID)
  
  # Create balanced panel
  balanced_panel <- balanced_data %>%
    filter(
      Classroom.ID %in% complete_teachers,
      biweek %in% analysis_biweeks
    )

  plot <- plot_action_percentages(
    balanced_panel, wrapper
    )
  action_percentage_balanced_plots[[paste(wrapper, sep = "_")]] <- plot
}

action_percentage_balanced_plots
...
#| fig-cap: "Correlation matrix and distributions of reinforcement learning parameters and model fit. The figure illustrates the pairwise Spearman correlations between key reinforcement learning parameters and model fit (AIC) derived from Q-learning model. The diagonal shows the distribution of each parameter, with histograms for discrete variables and density plots for continuous variables. The lower triangle displays scatterplots of pairwise relationships, with locally weighted scatterplot smoothing (LOWESS) lines in blue. The upper triangle presents correlation coefficients, with asterisks indicating statistical significance (*p < 0.05, **p < 0.01, ***p < 0.001)."

# Select the RL parameters and outcome variables
model_path <- "CBM/zearn_results/aggr_results/lap_aggr_q_model_2.mat"
df_corr_rl <- load_parameters(model_path)
df_corr_rl <- df_corr_rl %>%
  inner_join(AIC_data, by = "Classroom.ID") %>%
  dplyr::select(alpha, gamma, tau, cost, ev_init, AIC) %>%
  # Remove tau and cost outliers
  filter(tau < quantile(df_corr_rl$tau, probs = .75),
         cost < quantile(df_corr_rl$cost, probs = .95)) %>%
