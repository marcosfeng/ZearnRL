...
![Example of Teaching with Visual Models on Zearn](images/zearn-poster.jpg){#zearn-poster fig-align="center"}
...
Q-learning is a model-free reinforcement learning algorithm designed to learn a policy, which tells an agent what action to take under what circumstances (Watkins, C. J. C. H., & Dayan, P. (1992). Q-learning. Machine Learning, 8(3-4), 279-292). It does not require a model of the environment (hence the connotation "model-free"), and it can handle problems with stochastic transitions and rewards without requiring adaptations.

In the context of Q-learning, the agent interacts with the environment to gain experience and uses this experience to update its knowledge about the quality of particular actions, given the state of the environment. This knowledge is represented in the Q-values, a prediction of the future reward expected after taking an action in the current state (which in our context can be compared to subjective value, or a utility). The goal of Q-learning is to accurately learn these Q-values by updating them iteratively (Rummery, G. A., & Niranjan, M. (1994). On-line Q-learning using connectionist systems (Vol. 37). University of Cambridge, Department of Engineering).
...
### State-free model vs. model with states

In the state-free model, the Q-function is a function of the action only, and does not take into account the state of the environment (Sutton, R. S., & Barto, A. G. (2018). Reinforcement learning: An introduction. MIT press). This is a simplifying assumption that can be useful in situations where the state of the environment does not have a significant impact on the outcome of the action.

In contrast, the model with states takes into account the state of the environment when updating the Q-function. This allows the model to capture more complex dynamics, where the outcome of an action depends not only on the action itself but also on the state of the environment.

In this Q-learning model, the reward at a given week is given as

R(a\|St)=Badges(St+1\|a)-cost(a)

where is a discount factor, cost(not work)=0, and cost(work)=c is a free parameter.

### The kernel function

The kernel function is a mathematical tool used to measure the similarity between different states or actions. In the context of the Q-learning model with states and a kernel, the kernel function is used to compute a weighted average of the rewards obtained in similar state-action pairs in the past (Dayan, P., & Niv, Y. (2008). Reinforcement learning: the good, the bad and the ugly. Current opinion in neurobiology, 18(2), 185-196 https://doi.org/10.1023/A:1017928328829 https://doi.org/10.48550/arXiv.2204.10349). This allows the model to generalize from past experience and to make more informed decisions about future actions.

### Actor-Critic Model

The Actor-Critic model is a type of reinforcement learning algorithm that separates the task of selecting actions (the "actor") from the task of evaluating those actions (the "critic"). This separation allows the model to learn more efficiently by using the critic to guide the learning of the actor. The actor uses the information from the critic to update its policy, and the critic uses the information from the actor to update its value function.

The Actor-Critic model is different from the Q-learning model in several ways:

1.  Separation of Action and Evaluation: In Q-learning, the same function (the Q-function) is used to both select actions and evaluate those actions. In the Actor-Critic model, these tasks are separated into two different functions: the actor (policy function) and the critic (value function).

2.  Policy and Value Function: The Actor-Critic model uses a policy function (the actor) to select actions and a value function (the critic) to evaluate those actions. The Q-learning model, on the other hand, uses a single Q-function to both select and evaluate actions.

3.  Learning Efficiency: The Actor-Critic model can learn more efficiently than the Q-learning model because it uses the critic to guide the learning of the actor. This allows the actor to focus on improving its policy based on the feedback from the critic, rather than having to learn from trial and error.

In the Zearn context, the Actor-Critic model estimates several variables:

1.  Cost in badge-units for each component.

2.  Discount rate ($\gamma$).

3.  **Alpha**: These are the step-sizes for the update equations. They determine how much the weights are updated in each step of the learning process. A larger alpha means that the weights are updated more in each step, which can lead to faster learning but may also cause the model to overshoot the optimal solution.

The weights and policy are updated based on the prediction error and the outcome minus the cost, according to The equations for updating the weights (w) and policy (theta) are as follows:

w\[, j\] += alpha\[1\] \* delta \* to_vector(current_state); theta\[, j\] += alpha\[2\] \* delta \* to_vector(current_state);

where delta is the prediction error, alpha\[1\] and alpha\[2\] are the step-sizes, and current_state is the current state of the model.
...
In the context of predicting repeated behavior, RL algorithms can be used to model the decision-making process of individuals or groups by learning from the patterns in their actions and the resulting outcomes. The RL framework is particularly suitable for modeling teacher decision-making as it captures the dynamic and sequential nature of teaching. For instance, the state could be represented by the current progress of students in the class, and the actions could include assigning additional practice, providing personalized feedback, or adjusting lesson plans. The rewards could be improved student performance, increased student engagement, or reduced learning gaps. This interaction can be represented as:
...
The flexibility and robustness of the RL framework make it adaptable to changes in the learning environment and individual student needs. It allows for the incorporation of various state, action, and reward variables, and can be tailored to different educational contexts and objectives. Moreover, RL can capture the inherent uncertainty in the teaching environment. Teachers often face uncertainty about the best course of action to take to maximize student learning. RL models can capture this uncertainty and provide a framework for making optimal decisions under uncertainty. This is particularly relevant in the context of teaching, where the effectiveness of a teaching strategy may not be immediately apparent, and the optimal strategy may change as students' understanding evolves.
...
The Dynamic Analysis method, as proposed by Lau and Glimcher (2005), is a powerful tool for understanding choice behavior in the context of reinforcement learning. This method is particularly useful in tasks where reinforcement contingencies vary within a session, as it allows for the analysis of behavior at a granular, response-by-response level, while also providing insights into behavior averaged over multiple trials.

The Dynamic Analysis method is based on the premise that both past reinforcers and past choices significantly influence an individual's choices on each trial. This is a departure from simple reinforcement learning models, which often fail to account for the effects of past choices, leading to an incomplete characterization of behavior.

Dynamic Analysis provides a nuanced understanding of choice allocation. It implies that the ratio of responses to different alternatives matches the ratio of reinforcements obtained from these alternatives. The method allows for the prediction of response rate using weighted sums of past reinforcers, a technique that has been applied to predict choice allocation in concurrent schedules on both short (response-by-response) and longer (within and between sessions) time scales.
...
-   $J(t, t')$ is the Jaccard similarity between the current and past state-action pairs.

The Jaccard similarity is computed as follows:

$$
J(t, t') = 0.5 \times \text{choice}[t] \times \text{choice}[t'] + 0.5 \times \text{state}[t] \times \text{state}[t']
$$ {#eq-jaccard}

where:

-   $\text{choice}_t$ and $\text{choice}_t'$ are the choices made at the current and past time steps, respectively,
-   $\text{state}_t$ and $\text{state}_t'$ are the states at the current and past time steps, respectively.

### Implementing into the Zearn context

In the context of Zearn, the Q-learning model is used to predict the actions of teachers based on their past actions and the rewards they received. The reward is assumed to be a linear function of the number of badges earned, with the learning rate $\alpha$ as the multiplier and the cost as the constant subtracted from the reward. The discount rate $\gamma$ represents the degree to which future rewards are discounted compared to immediate rewards, and the inverse temperature $\tau$ controls the degree of randomness in the choice behavior. The estimated parameters in the Zearn context have the following interpretations:

-   Reward: In the context of Zearn, the reward is a linear function of the number of badges earned. This means that the more badges a teacher earns, the higher the reward. The learning rate determines how quickly the agent updates its Q-values in response to new information, while the cost represents the perceived effort or inconvenience associated with the action.

-   Cost: The cost parameter in the Q-learning model represents the perceived effort or inconvenience associated with the action. In the context of Zearn, this could represent the effort required to complete a particular task or the inconvenience of deviating from a preferred teaching method. A high cost means that the action is perceived as difficult or inconvenient, which makes it less likely to be chosen.

-   Learning rate ($\alpha$): The learning rate determines the extent to which the newly acquired information will override the old information. A factor of 0 will make the agent not learn anything.

-   Discount rate ($\gamma$): The discount rate represents the degree to which future rewards are discounted compared to immediate rewards. A high discount rate means that future rewards are considered almost as valuable as immediate rewards, which encourages long-term planning. A low discount rate means that immediate rewards are much more valuable than future rewards, which encourages short-term thinking (Doya, K. (2002). Metalearning and neuromodulation. Neural Networks, 15(4-6), 495-506).

-   Inverse temperature ($\tau$): The inverse temperature controls the degree of randomness in the choice behavior. A high inverse temperature means that the agent is more likely to choose the action with the highest expected reward, while a low inverse temperature means that the agent is more likely to choose actions randomly. This parameter can be interpreted as a measure of the agent's confidence in its Q-values, or the trade-off between choosing actions that are known to yield high rewards (exploitation) and actions that might yield higher rewards (exploration) (Glaeser, S., & Gläscher, J. (2010). Dynamic adjustment of the speed--accuracy trade-off in value-based decisions. Journal of Neuroscience, 30(29), 9655-9662 Mnih, V., Kavukcuoglu, K., Silver, D., Rusu, A. A., Veness, J., Bellemare, M. G., ... & Petersen, S. (2015). Human-level control through deep reinforcement learning. Nature, 518(7540), 529-533).

-   State: In the state-dependent model, the state represents the current situation or context in which the decision is being made. This could include factors such as the current week of the course, the current topic being taught, or the current performance level of the students. The state can influence the expected reward of different actions and thus affect the decision-making process.
