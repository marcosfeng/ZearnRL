...
df_poverty$Percentage <- as.numeric(gsub("%", "", df_poverty$Percentage))

# Create Bar Graph for Poverty Level
ggplot(df_poverty, aes(x = Variable, y = Percentage, fill = Variable)) +
...
# Convert Percentage to numeric
df_income$Percentage <- as.numeric(gsub("%", "", df_income$Percentage))

# Create Bar Graph for Income Distribution
ggplot(df_income, aes(x = Variable, y = Percentage, fill = Variable)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  scale_y_continuous(labels = scales::percent_format()) +
