...
Our dataset comprises of classrooms spanning a period of around 40 weeks. Given the limited amount of data per classroom, individual-level estimation using maximum likelihood estimation would yield noisy results. Although group-level estimation provides reliable estimates, it overlooks individual differences, which are crucial to our analysis.
...
#| tbl-cap: "Model comparison using Leave-One-Out Information Criterion (LOOIC)"
library(brms)
library(loo)
...
df_looic %>%
  ...
  tab_footnote(
    footnote = "LOOIC values, a measure of model quality, were calculated for each model type. Lower values indicate better model performance. Q-learning models were built using a kernel-based approach. Hierarchical models incorporate a hierarchical structure to account for classroom-level variations.",
...
In order to understand the factors that contribute to a teacher's ability to maximize lesson completion, we analyzed teachers' performance across various parameters. We focused on the correlation between these parameters and the average weekly badges earned per teacher, which we used as a proxy for lesson completion.
...
## Impact of RL Parameters on Teacher Behavior and Student Achievement

Our second research question was related to how the parameters of RL models vary with teacher behavior and the effects of these variations on student achievement. We found that the RL parameters, particularly the learning rate and the exploration-exploitation trade-off, significantly influence teacher behavior and student achievement.

Teachers with a higher learning rate, indicating a greater ability to adapt their teaching strategies based on feedback, were associated with higher student achievement. Similarly, teachers who effectively balanced exploration (trying new teaching strategies) and exploitation (sticking with known effective strategies) also had students with better outcomes. These findings highlight the potential of RL models to inform teacher training programs and interventions to improve student achievement.

## Influence of Teacher Background, Training, and Experience

Our third research question asked how teacher background, training, and experience influence their adaptation to and implementation of the Zearn Math curriculum. These factors significantly affect how teachers interact with the Zearn Math platform and their students.

For instance, veteran teachers with substantial pedagogical content knowledge (PCK) tended to rely on their teaching experiences rather than learning from Zearn Math. In contrast, new teachers, especially those without formal training, struggled to adapt to new practices and had weaker PCK. These findings suggest that teacher background, training, and experience are crucial when designing and implementing new programs like Zearn Math.
...
## Graphical Represnetation of the Bayesian Hierarchical Model

```{r, engine = 'tikz'}
\begin{tikzpicture}

% Nodes for group-level parameters
\node[obs] (muCost) {$\mu_{\text{cost}}$};
\node[obs, right=of muCost] (sigmaCost) {$\sigma_{\text{cost}}$};
\node[obs, below=of muCost] (muGamma) {$\mu_{\gamma}$};
\node[obs, right=of muGamma] (sigmaGamma) {$\sigma_{\gamma}$};
\node[obs, below=of muGamma] (muAlpha) {$\mu_{\alpha}$};
\node[obs, right=of muAlpha] (sigmaAlpha) {$\sigma_{\alpha}$};
\node[obs, below=of muAlpha] (muTau) {$\mu_{\tau}$};
\node[obs, right=of muTau] (sigmaTau) {$\sigma_{\tau}$};

% Nodes for individual-level parameters
\node[latent, right=of sigmaCost] (cost) {$\text{cost}_{i,j}$};
\node[latent, right=of sigmaGamma] (gamma) {$\gamma_i$};
\node[latent, right=of sigmaAlpha] (alpha) {$\alpha_i$};
\node[latent, right=of sigmaTau] (tau) {$\tau_i$};

% Connecting nodes
\draw[->] (muCost) -- (cost);
\draw[->] (sigmaCost) -- (cost);
\draw[->] (muGamma) -- (gamma);
\draw[->] (sigmaGamma) -- (gamma);
\draw[->] (muAlpha) -- (alpha);
\draw[->] (sigmaAlpha) -- (alpha);
\draw[->] (muTau) -- (tau);
\draw[->] (sigmaTau) -- (tau);
...
# Initialize an empty data frame to store the group parameter estimates
group_params_df <- data.frame()
...
for (result_file in results_files) {
  ...
  # Add the group parameter estimates to the overall data frame
  group_params_df <- rbind(group_params_df, group_params)
}
