...
gt_table <- gt(summary_table,
  ...
  tab_header(
    title = "Summary Statistics by Grade Level"
  ) %>%
  cols_label(
    `Minutes per Student` = "Minutes per Student",
    `Badges per Student` = "Badges per Student",
    `Tower Alerts per Lesson Completion` = "Tower Alerts per Lesson Completion",
    `Minutes per Teacher` = "Minutes per Teacher"
  )
...
df_heatmap_teachers <- df_heatmap_teachers %>% arrange(-`Comp 1`)
df_heatmap_students <- df_heatmap_students %>% arrange(-`Comp 1`)
...
create_formula <- function(method, lag, comp) {
  ...
  for (i in 1:lag) {
    # Add the interaction term for Badges.per.Active.User_i * method_i
    terms <- c(terms, paste0("Badges.per.Active.User_", i, "*", method, comp, "_", i))
    if (i != lag) {
      for (j in (i + 1):lag) {
        # Add the interaction for Badges_i * method_j when i < lag
        terms <- c(terms, paste0("Badges.per.Active.User_", i, "*",
                                 method, comp, "_", j))
      }
    }
  }
  
  formula_string <- paste0(method, comp, " ~ ", paste(terms, collapse = " + "), " + ", "week_lag")
...
methods <- c("FrobeniusNNDSVD")
columns <- c("Badges.per.Active.User", "FrobeniusNNDSVD")
...
# Panel data
train_data <- pdata.frame(
  df[set == "train"], index = c("Classroom.ID", "week", "Teacher.User.ID")
  )
test_data <- pdata.frame(
  df[set == "test"], index = c("Classroom.ID", "week", "Teacher.User.ID")
  )

# Estimation
cl <- makeCluster(detectCores())
registerDoParallel(cl)
params <- expand.grid(col = c(methods),
                      lag = lags,
                      comp = 1:n_comp) %>%
...
results_df <- results_df %>%
  mutate(Method = as.factor(Method),
         Component = as.factor(unlist(Component)),
         Lag = as.numeric(Lag),
...
# Create a function to generate a plot for a given component and metric
generate_plot <- function(data, component) {
  filtered_data <- filter(data, Component == component)
  plot <- ggplot(filtered_data, aes(x = Lag, y = value)) +
    geom_line() +
...
plots <- list()
for (component in levels(plot_data$Component)) {
  plots[[paste("BIC", component)]] <-
    generate_plot(plot_data %>% rename(value = avg_bic), component)
  plots[[paste("Negative Log-Likelihood", component)]] <-
    generate_plot(plot_data %>% rename(value = avg_nloglik), component)
  plots[[paste("R-square", component)]] <-
    generate_plot(plot_data %>% rename(value = rsq), component)
}
...
combined_plot <- grid.arrange(
    grobs = plots,
    ncol = 3
    # top = "Comparative Analysis of Components"
)

```

```{r action-reward-corr}

# Select relevant variables for correlation analysis
rows <- c("Active.Users...Total", "Sessions.per.Active.User", 
          "Minutes.per.Active.User", "Badges.per.Active.User",
          "Boosts.per.Tower.Completion", "Tower.Alerts.per.Tower.Completion")
columns_1 <- c("FrobeniusNNDSVD1", "FrobeniusNNDSVD1_1",
               "FrobeniusNNDSVD1_2", "FrobeniusNNDSVD1_3")
columns_2 <- c("FrobeniusNNDSVD2", "FrobeniusNNDSVD2_1",
               "FrobeniusNNDSVD2_2", "FrobeniusNNDSVD2_3")
columns_3 <- c("FrobeniusNNDSVD3", "FrobeniusNNDSVD3_1",
               "FrobeniusNNDSVD3_2", "FrobeniusNNDSVD3_3")

selected_vars <- df %>%
  select(all_of(c(rows, columns_1, columns_2, columns_3)))


# Compute correlation matrix
cor_matrix <- cor(selected_vars, use = "complete.obs", method = "spearman")

# Function to test significance and annotate correlation matrix
annotate_cor_matrix <- function(cor_matrix, n) {
  cor_matrix_melted <- melt(cor_matrix)
  p_values <- apply(cor_matrix_melted, 1, function(x) {
    cor.test(selected_vars %>% pull(as.character(x[1])),
             selected_vars %>% pull(as.character(x[2])),
             method = "spearman")$p.value
    })
  cor_matrix_melted <- cor_matrix_melted %>%
    mutate(coeff = if_else(p_values < 0.001 / n, # Bonferroni
                           sprintf("%.2f", value),
                           paste0(sprintf("%.2f", value), "†")))
  return(cor_matrix_melted)
}

# Prepare data for plotting
cor_matrix_1 <- cor_matrix[rows, columns_1]
cor_matrix_2 <- cor_matrix[rows, columns_2]
cor_matrix_3 <- cor_matrix[rows, columns_3]

n <- length(cor_matrix) # Number for Bonferroni correction
melted_cor_matrix_1 <- annotate_cor_matrix(cor_matrix_1, n) %>%
    mutate(Var1 = case_when(
      Var1 == "Active.Users...Total" ~ "Active Students",
      Var1 == "Sessions.per.Active.User" ~ "Student Logins",
      Var1 == "Minutes.per.Active.User" ~ "Student Minutes",
      Var1 == "Badges.per.Active.User" ~ "Student Badges",
      Var1 == "Boosts.per.Tower.Completion" ~ "Student Boosts",
      Var1 == "Tower.Alerts.per.Tower.Completion" ~ "Student Alerts"
      ))
melted_cor_matrix_2 <- annotate_cor_matrix(cor_matrix_2, n) %>%
    mutate(Var1 = case_when(
      Var1 == "Active.Users...Total" ~ "Active Students",
      Var1 == "Sessions.per.Active.User" ~ "Student Logins",
      Var1 == "Minutes.per.Active.User" ~ "Student Minutes",
...
melted_cor_matrix_3 <- annotate_cor_matrix(cor_matrix_3, n) %>%
    mutate(Var1 = case_when(
      Var1 == "Active.Users...Total" ~ "Active Students",
      Var1 == "Sessions.per.Active.User" ~ "Student Logins",
      Var1 == "Minutes.per.Active.User" ~ "Student Minutes",
...
# Modified plot function with conditional y-axis labels
plot_cor_matrix <- function(melted_matrix, title, show_y_axis = TRUE,
                            xlabel = c("t", "t-1", "t-2", "t-3")) {
  p <- ggplot(data = melted_matrix, aes(x = Var2, y = Var1, fill = value)) +
    geom_tile() +
    geom_text(aes(label = coeff), color = "black", size = 3) +
...
p1 <- plot_cor_matrix(melted_cor_matrix_1, "Lag(NMF 1)", show_y_axis = TRUE)
p2 <- plot_cor_matrix(melted_cor_matrix_2, "Lag(NMF 2)", show_y_axis = FALSE)
p3 <- plot_cor_matrix(melted_cor_matrix_3, "Lag(NMF 3)", show_y_axis = FALSE)

# Create a dummy plot for extracting the legend
legend_plot <- ggplot(data.frame(value = c(-1, 1)),
...
# Extract the legend
legend <- ggplotGrob(legend_plot)$grobs[[
  which(sapply(ggplotGrob(legend_plot)$grobs,
               function(x) x$name) == "guide-box")
  ]]

# Combine the plots into one panel with a shared legend
grid.arrange(p1, p2, p3, nrow = 1, bottom = legend, widths = c(1.5, 1, 1))

```

```{r reward-action-corr}
...
# Create lags
for (col in rows) {
  for (lag_period in 1:3) {
    df <- get_lag_value(df, col, lag_period)
  }
}

rows <- c(rows, names(
  df[,Active.Users...Total_1:Tower.Alerts.per.Tower.Completion_3]
  ),
  names(
    df[,Badges.per.Active.User_1:Badges.per.Active.User_3]
  ))
rows <- rows[order(rows)]

columns <- c("FrobeniusNNDSVD1", "FrobeniusNNDSVD2", "FrobeniusNNDSVD3")

selected_vars <- df %>%
  select(all_of(c(rows, columns)))
...
# Prepare data for plotting
cor_matrix <- cor_matrix[rows, columns]
n <- length(cor_matrix) # Number for Bonferroni correction
melted_cor_matrix <- annotate_cor_matrix(cor_matrix, n) %>%
  mutate(
    Var2 = case_when(
      grepl("_1", Var1) ~ paste0(Var2, "_1"),
      grepl("_2", Var1) ~ paste0(Var2, "_2"),
      grepl("_3", Var1) ~ paste0(Var2, "_3"),
      .default = Var2),
    Var1 = case_when(
      grepl("Active.Users...Total", Var1) ~ "Lag(Active Students)",
      grepl("Sessions.per.Active.User", Var1) ~ "Lag(Student Logins)",
      grepl("Minutes.per.Active.User", Var1) ~ "Lag(Student Minutes)",
      grepl("Badges.per.Active.User", Var1) ~ "Lag(Student Badges)",
      grepl("Boosts.per.Tower.Completion", Var1) ~ "Lag(Student Boosts)",
      grepl("Tower.Alerts.per.Tower.Completion", Var1) ~ "Lag(Student Alerts)")
    )


# Create individual plots
p1 <- plot_cor_matrix(melted_cor_matrix %>% filter(grepl("D1", Var2)),
                      "NMF1", show_y_axis = TRUE)
p2 <- plot_cor_matrix(melted_cor_matrix %>% filter(grepl("D2", Var2)),
                      "NMF2", show_y_axis = FALSE)
p3 <- plot_cor_matrix(melted_cor_matrix %>% filter(grepl("D3", Var2)),
                      "NMF3", show_y_axis = FALSE)
...
# Combine the plots into one panel with a shared legend
grid.arrange(p1, p2, p3, nrow = 1, bottom = legend, widths = c(1.65, 1, 1))

```

```{r reward-minutes-corr}
...
columns <- c("Minutes.on.Zearn...Total", "Minutes.on.Zearn...Total_1",
             "Minutes.on.Zearn...Total_2", "Minutes.on.Zearn...Total_3")
# Create lags
for (lag_period in 1:3) {
  df <- get_lag_value(df, columns[1], lag_period)
}
selected_vars <- df %>%
  select(all_of(c(rows, columns)))
...
p1 <- plot_cor_matrix(
  ...
  mutate(
    ...
    Var1 = case_when(
      ...
      grepl("Boosts.per.Tower.Completion", Var1) ~ "Lag(Student Boosts)",
      grepl("Tower.Alerts.per.Tower.Completion", Var1) ~ "Lag(Student Alerts)")
    ), "Teacher Minutes", show_y_axis = TRUE)
p2 <- plot_cor_matrix(
  melted_cor_matrix %>%
    filter(!grepl("_", Var1)) %>%
  mutate(
    Var1 = case_when(
      grepl("Active.Users...Total", Var1) ~ "Active Students",
      grepl("Sessions.per.Active.User", Var1) ~ "Student Logins",
      grepl("Minutes.per.Active.User", Var1) ~ "Student Minutes",
      grepl("Badges.per.Active.User", Var1) ~ "Student Badges",
      grepl("Boosts.per.Tower.Completion", Var1) ~ "Student Boosts",
      grepl("Tower.Alerts.per.Tower.Completion", Var1) ~ "Student Alerts")
    ), "Lag(Teacher Minutes)", show_y_axis = TRUE)
...
# Combine the plots into one panel with a shared legend
grid.arrange(p1, p2, nrow = 1, bottom = legend, widths = c(1.055, 1))
...
create_RL_formula <- function(action, reward, state = NULL, lag = 1,
  ...
  for (i in 1:lag) {
    fe_terms <- c(fe_terms, paste0(reward, "_", i), paste0(action, "_", i))
...
# Function to compute out-of-sample negative log-likelihood for logistic regression
compute_nloglik_logit <- function(model, test_data, outcome_var) {
  # Predicted probabilities
  predicted_probs <- predict(model, newdata = test_data, type = "response",
                             allow.new.levels = TRUE)
  
  # Actual binary outcomes
  actual_outcomes <- test_data[[outcome_var]]
  
  # Compute log-likelihood
  log_likelihood <- actual_outcomes * log(predicted_probs) + 
                    (1 - actual_outcomes) * log(1 - predicted_probs)
  
  # Negative log-likelihood
  nll <- -sum(log_likelihood, na.rm = TRUE)
  
  return(as.numeric(nll))
}

# Data and Variables
actions <- c("Frobenius.NNDSVD_teacher", "Minutes.on.Zearn...Total")
rewards <- c("Frobenius.NNDSVD_student")
# rewards <- c("Active.Users...Total", "Sessions.per.Active.User",
#              "Minutes.per.Active.User", "Badges.per.Active.User",
#              "Boosts.per.Tower.Completion", "Tower.Alerts.per.Tower.Completion")
columns <- colnames(
  df %>% select(contains(c(actions, rewards)))
)

df_binary <- as.data.table(
  df %>%
    group_by(MDR.School.ID) %>%
    mutate(across(contains(actions), ~ if_else(. > median(.), 1, 0))) %>%
    ungroup()
  )
...
for (col in columns) {
  for (lag_period in 1:n_lags) {
    df_binary <- get_lag_value(df_binary, col, lag_period)
  }
}

# Create train and test datasets
train_data <- df_binary[set == "train"]
test_data <- df_binary[set == "test"]

# Estimation
params <- crossing(
  act = columns[str_detect(columns, paste0(actions, collapse = "|"))],
  lag = lags,
  rwd = columns[str_detect(columns, paste0(rewards, collapse = "|"))],
  st  = columns[str_detect(columns, paste0(rewards, collapse = "|"))]
) %>%
  filter(rwd != st) %>%
  unique()
...
results <- foreach(i = 1:nrow(params),
  ...
  model <- tryCatch(
    glmer(formula, data = train_data, family = binomial(link = "logit"),
          control = glmerControl(boundary.tol = 1e-3)),
  ...
  list(Action = act,
       ...
       nloglik = tryCatch(
         compute_nloglik_logit(model, test_data, act),
         error = function(e) { return(NULL) }
         ),
       bic = tryCatch(
         as.numeric(summary(model)$AICtab["BIC"]),
         error = function(e) { return(NULL) }
         )
       )
}
...
# Convert variables to appropriate data types
results_df <- results_df %>%
  filter(Action != "Minutes.on.Zearn...Total") %>%
  mutate(Action = as.factor(unlist(Action)),
         Reward = as.factor(unlist(Reward)),
         State = as.factor(unlist(State)),
         Lag = as.numeric(unlist(Lag)),
         nloglik = as.numeric(unlist(nloglik)),
         bic = as.numeric(unlist(bic)),
         # rsq = as.numeric(unlist(rsq)),
         # formula = as.character(unlist(equation)),
         across(dplyr::starts_with("coef"),
                ~ as.numeric(unlist(.)))) %>%
  mutate(Action = case_when(
    Action == "Frobenius.NNDSVD_teacher1" ~ "Teacher NMF1",
    Action == "Frobenius.NNDSVD_teacher2" ~ "Teacher NMF2",
    Action == "Frobenius.NNDSVD_teacher3" ~ "Teacher NMF3",
    Action == "Frobenius.NNDSVD_teacher4" ~ "Teacher NMF4",
    Action == "Minutes.on.Zearn...Total" ~ "Teacher Minutes"
    ),
    across(c(Reward, State),
           ~ case_when(
             . == "Frobenius.NNDSVD_student1" ~ "Student NMF1",
             . == "Frobenius.NNDSVD_student2" ~ "Student NMF2",
             . == "Frobenius.NNDSVD_student3" ~ "Student NMF3",
             . == "Frobenius.NNDSVD_student4" ~ "Student NMF4")
    )) %>%
  na.omit()
  
# Function to create a plot for a specific action
plot_for_action <- function(action_var) {
  ggplot(results_df[results_df$Action == action_var, ],
         aes(x = nloglik, y = bic, color = Reward, shape = State)) +
    geom_point() +
    theme_minimal() +
    labs(title = action_var,
         x = "Out-of-sample Neg. Log-likelihood", y = "BIC") +
    theme(legend.position = "none")
}

# Create individual plots for each action
plots <- lapply(unique(results_df$Action),
                plot_for_action)

# Extract separate legends for color and shape
legend_color_plot <- ggplot(
  results_df, aes(x = nloglik, y = bic, color = Reward)
  ) +
  geom_point() +
  theme_void() +
  theme(legend.position = "bottom")
...
legend_shape_plot <- ggplot(
  results_df, aes(x = nloglik, y = bic, shape = State)
  ) +
  geom_point() +
  theme_void() +
  theme(legend.position = "bottom")

legend_color <- ggplotGrob(legend_color_plot)$grobs[[
  which(sapply(ggplotGrob(legend_color_plot)$grobs,
               function(x) x$name) == "guide-box")
  ]]
legend_shape <- ggplotGrob(legend_shape_plot)$grobs[[
  which(sapply(ggplotGrob(legend_shape_plot)$grobs,
               function(x) x$name) == "guide-box")
  ]]

# Combine the color and shape legends vertically
combined_legend <- arrangeGrob(legend_color, legend_shape, ncol = 1)

# Combine the plots into a 2x2 panel with the combined legends at the bottom
combined_plot <- grid.arrange(
  grobs = c(plots, list(combined_legend)),
...
cl <- makeCluster(min(detectCores() - 1, nrow(params)))
registerDoParallel(cl)
top_results <- foreach(i = 1:nrow(params),
  ...
  st  <- as.character(params$st[i])
  formula <- create_RL_formula(act, rwd, st, lag,
                               group = "Teacher.User.ID/MDR.School.ID")
...
# Calculating variance over time for each teacher
teacher_variance <- df %>%
  mutate(poverty = factor(poverty, ordered = T),
         income = factor(income, ordered = T),
         across(c(FrobeniusNNDSVD1:FrobeniusNNDSVD3),
                ~ ifelse(. > median(.), 1, 0))) %>%
  group_by(Teacher.User.ID) %>%
  summarise(
    NMF1_sd = sd(FrobeniusNNDSVD1, na.rm = TRUE),
    NMF2_sd = sd(FrobeniusNNDSVD2, na.rm = TRUE),
    NMF3_sd = sd(FrobeniusNNDSVD3, na.rm = TRUE),
    NMF1_var = var(FrobeniusNNDSVD1, na.rm = TRUE),
    NMF2_var = var(FrobeniusNNDSVD2, na.rm = TRUE),
    NMF3_var = var(FrobeniusNNDSVD3, na.rm = TRUE),
    num_classes = median(teacher_number_classes),
    num_students = sum(Students...Total),
    poverty = first(poverty),
    income = first(income),
    charter.school = median(charter.school),
    school.account = median(school.account),
    n_weeks = mean(n_weeks)
  ) %>%
  # Remove teachers with no variance
  filter(NMF1_var > 0 & NMF2_var > 0 & NMF3_var > 0) %>%
  # Remove teachers with biased variance
  filter(NMF1_var <= 0.25 & NMF2_var <= 0.25 & NMF3_var <= 0.25)

# Calculating variance over time for each classroom
classroom_variance <- df %>%
  mutate(poverty = factor(poverty, ordered = T),
         income = factor(income, ordered = T),
         across(c(Active.Users...Total:Tower.Alerts.per.Tower.Completion),
                ~ scale(.))) %>%
  group_by(Classroom.ID) %>%
  summarise(
    Active_sd = sd(Active.Users...Total, na.rm = TRUE),
    Minutes_sd = sd(Minutes.per.Active.User, na.rm = TRUE),
    Badges_sd = sd(Badges.per.Active.User, na.rm = TRUE),
    Boosts_sd = sd(Boosts.per.Tower.Completion, na.rm = TRUE),
    Tower_sd = sd(Tower.Alerts.per.Tower.Completion, na.rm = TRUE),
    num_classes = median(teacher_number_classes),
    num_students = sum(Students...Total),
    poverty = first(poverty),
    income = first(income),
  ...
  filter(Boosts_sd > 0 & Tower_sd > 0)

# Density plot for teacher variables
p1 <- ggplot(teacher_variance) +
  geom_density(aes(x = NMF1_sd, fill = "NMF1_sd"), alpha = 0.5) +
  geom_density(aes(x = NMF2_sd, fill = "NMF2_sd"), alpha = 0.5) +
  geom_density(aes(x = NMF3_sd, fill = "NMF3_sd"), alpha = 0.5) +
  scale_fill_brewer(palette = "Dark2") +
  labs(title = "Density Plot of Teacher Variables", x = "St Dev", y = "Density") +
  theme_minimal() +
  theme(legend.title = element_blank())

# Density plot for student variables
p2 <- ggplot(classroom_variance) +
  geom_density(aes(x = Active_sd, fill = "Active_sd"), alpha = 0.5) +
  geom_density(aes(x = Minutes_sd, fill = "Minutes_sd"), alpha = 0.5) +
  geom_density(aes(x = Badges_sd, fill = "Badges_sd"), alpha = 0.5) +
  # geom_density(aes(x = Boosts_sd, fill = "Boosts_sd"), alpha = 0.5) +
  geom_density(aes(x = Tower_sd, fill = "Tower_sd"), alpha = 0.5) +
  scale_fill_brewer(palette = "Dark2") +
  labs(title = "Density Plot of Student Variables", x = "St Dev", y = "Density") +
  theme_minimal() +
  theme(legend.title = element_blank())
...
# Fitting the model using brm function
fit_mv_model <- brm(NMF1_var ~ num_students, 
                    data = teacher_variance, 
                    family = Beta(), 
                    chains = 8,
                    cores = 8,
                    iter = 2000,
                    warmup = 1000, 
                    backend = "cmdstanr")
fit_mv_model2 <- brm(NMF2_sd ~ n_weeks + num_classes +
                       school.account + n_weeks, 
                    data = teacher_variance, 
                    family = Beta(), 
                    chains = 6,
                    cores = 6,
                    ...
                    silent = 2,
                    backend = "cmdstanr")
fit_mv_model <- brm(mvbind(NMF1_sd, NMF3_sd) ~ n_weeks + num_classes +
                       school.account + n_weeks,
                    data = teacher_variance, 
                    family = Beta(), 
                    chains = 6,
...
# Preparing data for correlogram
chart.Correlation(teacher_variance %>%
                    mutate(poverty = as.integer(poverty),
                           income = as.integer(income)) %>%
                    select(NMF1_sd, NMF2_sd, NMF3_sd,
                           num_students, poverty, income,
                           charter.school, school.account, n_weeks),
                  histogram = TRUE, method = "spearman")
                           


chart.Correlation(classroom_variance %>%
                    select(NMF1_var, NMF2_1_var, NMF3_1_var,
                           
                           Active_sd, Minutes_sd, Badges_sd, Boosts_sd, Tower_sd,
                           num_classes, num_students,
                           poverty, income, charter.school, school.account, n_weeks), histogram = TRUE, method = "spearman",
                  pch = 20, cex = 0.5, col = rgb(0, 0, 1, 0.5))
cor_data <- teacher_variance %>%
  select(NMF1_var, NMF2_1_var, NMF3_1_var, num_classes, num_students, poverty, income, charter.school, school.account, n_weeks)
...
TO-DO: temporal analysis of teachers choices and a bunch of variables: 
bayesian analysis of 
<!-- Panel Regression of all variables and badges. -->
<!-- Investigate the basic analysis of behavior and its structure over time. John: "Really try to dig into what the structure is between how teachers' choices are changing across weeks, and what factors might be related to those." -->
<!-- Detailed Data Analysis by School and Classroom: how much do individual schools and classrooms vary. You might want to downsample schools in some way. -->
<!-- Cooper: "What is going on in the data—school quality, what is changing over time, etc. just understand the statistics of all the data." -->

<!-- Temporal Changes in States and Rewards -->
<!-- "Are these like behavioral metrics like Tower Alerts decreasing as a function of time? Is it a product of teacher interventions, or are the teachers, for some reason, acting less at the end of the 15 weeks?" -->
