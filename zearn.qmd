...
evidence_df[7,1:3] <- c("Pedagogical Knowledge","Struggles",
                        "Activity") # AC 17
evidence_df[8,1:3] <- c("Pedagogical Knowledge","Badges",
                        "No. Students") # AC 2
evidence_df[9,1:3] <- c("Pedagogical Knowledge","Struggles",
                        "No. Students, Activity") # AC 23
evidence_df[10,1:3] <- c("Pedagogical Knowledge","Struggles",
                        "Badges, No. Students, Activity") # AC 27
evidence_df[11,1:3] <- c("Pedagogical Knowledge","No. Students",
                         "Badges") # AC 29
evidence_df[12,1:3] <- c("Pedagogical Knowledge","Badges",
                         "Activity") # AC 3
...
evidence_df[18,1:3] <- c("Pedagogical Knowledge","Activity",
...
evidence_df[21,1:3] <- c("Pedagogical Knowledge", "Badges",
...
top_models_table <- data.frame(
  ...
  Reward = c(rep("Badges", 2), rep("Struggles", 2), "No. Students", rep("Activity", 4)),
  State = c(NA, "No. Students", NA, "Badges, No. Students, Activity", "Badges, Struggles", 
            "Badges", "Struggles, No. Students", NA, NA),
  ModelFrequency = c(model_freq_ql, model_freq_ac),
  LogLikelihood = c(BIC_ql, BIC_ac)
)
...
top_models_table %>%
  select(-Action) %>%
  group_by(Reward) %>%
  arrange(desc(LogLikelihood), Reward, State) %>%
  gt(row_group_as_column = TRUE) %>%
  tab_stubhead(label = "Reward") %>%
  cols_label(
  ...
  sub_missing() %>%
  fmt_number(columns = c(LogLikelihood), decimals = 1) %>%
...
log_likelihoods <- as.numeric(full_model[["cbm"]][[4]][[1]][[1]][, 2])
...
ggplot(data = data.frame(LogLikelihoods = log_likelihoods),
  ...
  geom_histogram(fill = brewer.pal(8, "Set2")[1],
                 color = brewer.pal(8, "Dark2")[1],
                 bins = 30, alpha = 0.8) +
  geom_vline(xintercept = median_log_likelihood,
             color = brewer.pal(8, "Dark2")[8], 
             linetype = "dashed", size = 1) +
  annotate("text", x = median_log_likelihood, y = 0, 
           label = paste0("Median: ", median_log_likelihood),
           color = brewer.pal(8, "Dark2")[8],
           vjust = 1.3, hjust = -0.1, size = 4) +
  theme_minimal() +
...
group_mean <- full_model[["cbm"]][[5]][[3]][[2]][[1]]
error_bar <- full_model[["cbm"]][[5]][[4]][[2]][[1]]
teacher_specific <- full_model[["cbm"]][[5]][[1]][[2]][[1]]
  ...
  cbind(
    data.frame(
      ModelType = factor(teacher_classification - min(teacher_classification), 
                         labels = c("Logit", "Actor-Critic"))
      )) %>%
  cbind(teacher_params)
names(heterogeneity)[3:9] <- c(
...
heterogeneity %>%
  dplyr::select(ModelType, `Charter School`, `Paid Account`, Poverty, 
         `No. of Weeks`, `Active Students`, `Avg. Badges`, `Avg. Tower Alerts`, 
         Action, Reward, State) %>%
  tbl_summary(
    by = ModelType,  # Grouping variable
...
teacher_params <- full_model[["cbm"]][[5]][[1]][[1]][[1]]
fit_group <- ifelse(log_likelihoods >= median_log_likelihood, "Good Fit", "Bad Fit")

# Load classroom data
load("CBM/data/classrooms_full.RData")
valid_subj <- readMat("CBM/zearn_results/top_results/valid_subj_all.mat")
valid_subj <- as.logical(unlist(valid_subj))

# Merge and transform the data
full_data <- as.data.frame(classrooms) %>%
  rename(`Classroom.ID` = classrooms) %>%
  filter(valid_subj) %>%
  cbind(teacher_params) %>%
  mutate(
    FitGroup = factor(fit_group, levels = c("Good Fit", "Bad Fit"))
  ) %>%
  left_join(
    df %>%
      group_by(Classroom.ID, Teacher.User.ID) %>%
      summarize(
        `Charter School` = mean(charter.school, na.rm = TRUE),
        `Paid Zearn Account` = mean(school.account, na.rm = TRUE),
        Action = mean(Frobenius.NNDSVD_teacher2, na.rm = TRUE),
        Reward = mean(Frobenius.NNDSVD_student1, na.rm = TRUE),
        State = mean(Frobenius.NNDSVD_student3, na.rm = TRUE),
        `Active Students` = mean(`Active.Users...Total`, na.rm = TRUE),
        `Avg. Badges` = mean(`Badges.per.Active.User`, na.rm = TRUE),
        `Avg. Tower Alerts` = mean(`Tower.Alerts.per.Tower.Completion`, na.rm = TRUE),
        Income = unique(income),
        Poverty = unique(poverty),
        `No. of Weeks` = n(),
...
full_data %>%
  dplyr::select(FitGroup, `Charter School`, `Paid Zearn Account`, Poverty, 
         `No. of Weeks`, `Active Students`, `Avg. Badges`, `Avg. Tower Alerts`, 
         Action, Reward, State) %>%
  tbl_summary(
...
# Prepare CBM hierarchical model results
names(full_data)[2:8] <- c("alpha_w", "alpha_theta", "gamma", "tau",
                           "theta_init", "w_init", "cost")
# full_data <- full_data %>%
#   mutate(across(c(alpha_w, alpha_theta, gamma), ~ 1/(1+exp(-.)))) %>%
#   mutate(across(c(tau, cost), ~ exp(.)))
...
model1 <- lm(`Avg. Badges` ~ alpha_w + alpha_theta + gamma + tau +
               cost + theta_init + w_init,
             data = full_data)
model2 <- lm(`Avg. Badges` ~ alpha_w + alpha_theta + gamma + tau +
               cost + theta_init + w_init +
               `Active Students` + n_classes_by_teacher,
             data = full_data)
model3 <- lm(`Avg. Badges` ~ alpha_w + alpha_theta + gamma + tau +
               cost + theta_init + w_init +
               `Active Students` + n_classes_by_teacher +
               grade + `No. of Weeks` + Poverty +
               `Charter School` + `Paid Zearn Account`,
             data = full_data)
model4 <- lm(`Avg. Tower Alerts` ~ alpha_w + alpha_theta + gamma + tau +
               cost + theta_init + w_init,
             data = full_data)
model5 <- lm(`Avg. Tower Alerts` ~ alpha_w + alpha_theta + gamma + tau +
               cost + theta_init + w_init  +
               `Active Students` + n_classes_by_teacher,
             data = full_data)
model6 <- lm(`Avg. Tower Alerts` ~ alpha_w + alpha_theta + gamma + tau +
               cost + theta_init + w_init +
               `Active Students` + n_classes_by_teacher +
               grade + `No. of Weeks` + Poverty +
               `Charter School` + `Paid Zearn Account`,
...
stargazer(model1, model2, model3, model4, model5, model6,
          ...
          header=FALSE,
          align = TRUE,
          covariate.labels = c("$\\alpha_w$", "$\\alpha_\\theta$",
                               "$\\gamma$", "$\\tau$",
                               "Cost", "$\\theta_{init}$", "$w_{init}$",
                               "No. of Active Students",
                               "No. of Classes", "Grade", "No. of Weeks",
                               "Poverty Level",
                               "Charter School", "Paid Zearn Account"),
...
responses <- c("Income", "Poverty", "`Charter School`", "`Paid Zearn Account`", "`No. of Weeks`")
