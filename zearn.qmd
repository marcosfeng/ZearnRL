...
df_pca <- as.data.frame(df_comp) %>%
  ...
  mutate(across(matches("RD\\.elementary_schedule:RD\\.pd_course_guide"), ~log1p(.)))
...
# Clean environment
rm(list = setdiff(ls(), c("df", "df_pca", "df_comp")))
...
# Number of components for NMF and LSTM
n_comp = min(X.shape) // 3
n_LSTM = min(X.shape) // 2
...
# Function for NMF
def nmf_method(n, method, initial, X_scaled):
    nmf = NMF(
      n_components=n,
      init=initial,
      beta_loss=method)
    ...
    results.setdefault(initial, {})[n] = X_nmf
    components.setdefault(initial, {})[n] = nmf_comp
    residuals.setdefault(initial, {})[n] = ((X_scaled - X_hat)**2).sum().sum()  # RSS
    silhouette.setdefault(initial, {})[n] = silhouette_score(nmf_comp.transpose(), labels)

# Call the function for NMF
for n in range(2, n_comp):
  for initial in {'nndsvd', 'nndsvda'}:
    try:
      nmf_method(n, 'frobenius', initial, X_scaled)
    except Exception:
      continue
...
def build_model(hp):
    ...
    # Add a variable number of hidden LSTM layers for decoder
    for i in range(hp.Int('num_layers', 1, 5)):
        x = LSTM(
          units=hp.Int('units_' + str(i), min_value=2, max_value=n_LSTM, step=1),
          activation='relu',
          return_sequences=True)(x)
    
    # Encoding layer
    encoding_units = hp.Int('encoding_units', min_value=2, max_value=n_comp, step=1)
    encoding = TimeDistributed(
      Dense(units=encoding_units, activation='relu', kernel_constraint=NonNeg()),
      name='encoding')(x)
    decoded = TimeDistributed(
      Dense(units=n_features, activation='sigmoid'),
      name='decoding')(encoding)
    label_output = Dense(1, activation='linear', name='label_output')(encoding)
...
```{r clean environment}
rm(list = setdiff(ls(), c("df", "minmax")))
```
