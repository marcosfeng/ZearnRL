...
df <- df %>%
  ...
  mutate(Badges.per.Active.User = scale(Badges.per.Active.User),
         Tower.Alerts.per.Tower.Completion = scale(Tower.Alerts.per.Tower.Completion)) %>%
  ungroup() %>%
  filter(!is.na(Badges.per.Active.User) &
         !is.na(Tower.Alerts.per.Tower.Completion)) %>%
  arrange(Classroom.ID, week)
...
df_heatmap <- components_list[["Frobenius NNDSVDA"]][["3"]] %>%
  t() %>% as.data.frame()
...
fit_mv_model <- brm(NMF1_var ~ num_students, 
                    ...
                    warmup = 5000, 
                    backend = "cmdstanr")
fit_mv_model2 <- brm(NMF1_sd ~ num_students, 
                    data = teacher_variance, 
                    family = Beta(), 
                    chains = 8,
                    cores = 8,
                    ...
                    warmup = 5000, 
                    backend = "cmdstanr")
fit_mv_model <- brm(mvbind(NMF1_sd, NMF2_sd, NMF3_sd) ~ 
