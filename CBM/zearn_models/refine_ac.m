...
for i = 1:length(fname)
    ...
    wrapper_filename = fullfile('wrappers', ['wrapper_function_' num '.m']);
    
    % Read the .m file
    file_contents = fileread(wrapper_filename);
    
    % Extract the outcome variable name
    outcome_match = regexp(file_contents, 'subj\.outcome\s*=\s*subj\.(\w+);', 'tokens');
    % Extract the entire string of state variables
    state_match = regexp(file_contents, 'subj\.state\s*=\s*\[(subj\.\w+\s*(?:,\s*subj\.\w+\s*)*)\];', 'match');
    ...
    if ~isempty(outcome_match) && ~isempty(state_match)
        ...
        model_desc{i} = sprintf('Outcome: %s, States: %s', (outcome_var), strjoin((state_vars), ', '));
    else
...
T = table(model_desc(:), log_evidence_non_sci(:), 'VariableNames', {'Model', 'Log Evidence'});
