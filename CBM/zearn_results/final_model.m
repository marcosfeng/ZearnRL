...
bic = nan(length(data),numel(hbi_idx));
for post_idx = 1:size(hbi_idx,1)
    hbi_model = load(fname_hbi{post_idx});
    success_filter = all(success(:,hbi_idx(post_idx,:)),2);
    for model_idx = 1:size(hbi_idx,2)
        wrapper = str2func( ...
            sprintf('wrapper_post_%d', ...
            idx(hbi_idx(post_idx,model_idx))));
    
        for j = 1:length(data)
            if ~success_filter(j), continue, end
            [loglik(j,((post_idx-1)*size(hbi_idx,2)+model_idx)), ...
                prob{j,((post_idx-1)*size(hbi_idx,2)+model_idx)}, ...
                choice] = wrapper( ...
                hbi_model.cbm.output.parameters{model_idx}( ...
                j - sum(~success_filter(1:j)),:), ...
                data{j});
            bic(j,((post_idx-1)*size(hbi_idx,2)+model_idx)) = ...
                -2*loglik(j,((post_idx-1)*size(hbi_idx,2)+model_idx)) + ...
                length(hbi_model.cbm.output.parameters{model_idx}( ...
                j - sum(~success_filter(1:j)),:)) * ...
                log(length(choice));
            roc{j,((post_idx-1)*size(hbi_idx,2)+model_idx)} = ...
                rocmetrics(choice, ...
                prob{j,((post_idx-1)*size(hbi_idx,2)+model_idx)}, [0,1], ...
                NumBootstraps=500,BootstrapOptions=statset(UseParallel=true));
            auc(j,((post_idx-1)*size(hbi_idx,2)+model_idx)) = ...
                roc{j,((post_idx-1)*size(hbi_idx,2)+model_idx)}.AUC(1,1);
            auc_conf(j,((post_idx-1)*size(hbi_idx,2)+model_idx)) = ...
                roc{j,((post_idx-1)*size(hbi_idx,2)+model_idx)}.AUC(3,1) - ...
                roc{j,((post_idx-1)*size(hbi_idx,2)+model_idx)}.AUC(2,1);
        end
    end
end
...
for post_idx = 1:size(hbi_idx,1)
    load(fname_hbi{post_idx});
    cbm.output.auc = ...
        auc(:,((post_idx-1)*size(hbi_idx,2))+(1:size(hbi_idx,2)));
    cbm.output.auc_conf = ...
        auc_conf(:,((post_idx-1)*size(hbi_idx,2))+(1:size(hbi_idx,2)));
    cbm.output.loglik = ...
        loglik(:,((post_idx-1)*size(hbi_idx,2))+(1:size(hbi_idx,2)));
    cbm.output.bic = ...
        bic(:,((post_idx-1)*size(hbi_idx,2))+(1:size(hbi_idx,2)));
    save(fname_hbi{post_idx},"cbm");
end
